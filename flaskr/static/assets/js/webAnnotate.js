...
class LabelImage {
	...
	SetImage = (src, memory=false) => {
		...
		_nodes.image.addEventListener('load', () => {
			...
			// Add by June, 2020年11月4日16:30:43
			this.iName = _nodes.image.src.split('/')[_nodes.image.src.split('/').length - 1]; 
			// Add code ends
			//获取原有节点
			...
			// Add by June, 2020年11月4日15:48:36
			if (true) {
				let label = window.location.search.split('=')[1];
				let imgFile = {
					'label': label,
					'filename': this.iName,
				};
				...
				$.ajax({
					type: 'GET',
					url: SCRIPT_ROOT + '/annotate/getJson',
					data: imgFile,
					async: false, 
					dataType: 'json', 
					success: function (response) {				
						if (response.status == "success") {
							console.log("标注 " + imgFile['filename'] + " 获取成功！");
							labelJson = response.labelJson;
						}
					},
				...
				if (labelJson) {
					memory = labelJson; 	// Update memory from labelJson
					this.Arrays.imageAnnotateMemory = labelJson;
					this.ReplaceAnnotateShow();
					this.RepaintResultList();
					this.Arrays.imageAnnotateMemory.forEach((memory,index) => {
						this.RecordOperation('add', '绘制', index, JSON.stringify(memory));
	...
	ReplaceAnnotateMemory = () => {
		...
		this.Arrays.imageAnnotateShower.map((item)=> {
			...
			let rectMask = {
				...
				// Add by June, 2020年11月4日15:45:55
				"iWidth": this.iWidth,
