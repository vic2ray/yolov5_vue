...
// Modify by June, 2020年11月4日11:11:09
...
function selectImage(index) {
	...
	taskName.innerText = imgFiles[index].name || imgFiles[index].split('/')[5];
...
function saveJson(data, filename) {
	// Add by June, 2020年11月4日11:22:42
	let label = window.location.search.split('=')[1];
	$.ajax({
		url: SCRIPT_ROOT + '/annotate/saveJson',
		data: JSON.stringify({
			'label': label,
			'filename': filename,
			'data': data
		...
		success: function (response) {
			...
			if (response.status == "success") alert("标注 " + filename + " 保存成功！")
		},
